...
/**
 * OPENCAST PLUGIN OVERRIDE ---------------------------------------------------
 *
 * This is a verbatim copy of ILIAS core's `public/assets/js/file.js`
 * (il.UI.Input.File) with ONE behavioural fix: the form is submitted only once
 * EVERY dropzone finished, instead of after the first one. Core increments its
 * completion counter unconditionally while merely starting each upload, so on a
 * form with several file inputs (the event upload form has video + one input per
 * subtitle language + thumbnail) the first upload to finish — typically the tiny
 * subtitle — submitted the form and the still-running video upload lost its id.
 *
 * The fix lives in submitCurrentFormHook() and processCurrentFormDropzones().
 * Loaded after core file.js on the event upload form and re-asserted on
 * $(document).ready() below so it wins regardless of script order. Keep in sync
 * with core file.js on ILIAS updates. See OpenCast #535.
 * ---------------------------------------------------------------------------
 */
...
(function ($, Input) {
	Input.File = (function ($) {
		...
		let initDropzoneEventListeners = function (dropzone) {
			// Bind ONE shared submit handler per form. Passing the SAME function
			// reference lets the browser de-duplicate the repeated addEventListener
			// calls, so processCurrentFormDropzones() runs exactly once even though
			// every file input registers it on the same form. Core bound a fresh
			// arrow per dropzone (distinct references, not de-duplicated), so N file
			// inputs meant N synchronous runs that inflated the completion counter
			// and submitted the form before the uploads finished. See OpenCast #535.
			document.getElementById(dropzone.options.input_id)
				.closest('form')
				.addEventListener('submit', onFormSubmitHook);
		 ...
		 */
		let onFormSubmitHook = function (event) {
			// currentTarget is the <form> this single shared listener is bound to.
			let form = event.currentTarget;
			form.should_submit = true;

			event.preventDefault();
			setFormControlsDisabledState(form, true);
			processCurrentFormDropzones(form, event);
		}
		...
		let submitCurrentFormHook = function (dropzone) {
			// Submit the current form only once ALL dropzones finished. Every
			// finished dropzone increments the counter here on 'queuecomplete';
			// empty dropzones are counted in processCurrentFormDropzones(). This
			// replaces ILIAS core's unconditional per-loop increment, which let the
			// FIRST finished upload submit the form and dropped every still-running
			// upload's id (e.g. the video was lost when a small subtitle finished
			// first). See OpenCast #535.
		...
		let processCurrentFormDropzones = function (form, event) {
			...
			// Start counting fresh for this submit. Empty inputs are counted in the
			// loop below; inputs with a pending upload are counted on 'queuecomplete'
			// (submitCurrentFormHook). The form is submitted once both together reach
			// current_dropzone_count.
			current_dropzone = 0;
            ...
            if (typeof file_inputs[Symbol.iterator] === 'function') {
                ...
                for (let i = 0; i < file_inputs.length; i++) {
                    ...
                    // Use files.length (not getQueuedFiles()): a file already moved to
                    // "uploading" is no longer queued but must still count as pending,
                    // otherwise it would be miscounted as an empty input.
                    if (dropzone.files.length !== 0) {
                        // Counted later, once this dropzone's upload finishes
                        // (submitCurrentFormHook on 'queuecomplete').
                        dropzone.processQueue();
                    } else {
                        // Nothing to upload here: count it right away so it does
                        // not hold back the submit.
                        current_dropzone++;
                    }
                }
...
// Re-assert our fixed implementation at DOM-ready. The IIFE above already
// assigned il.UI.Input.File, but if this file happens to be parsed BEFORE ILIAS
// core's file.js, core would overwrite it again at its own parse time. Capturing
// the reference now and re-assigning it on ready guarantees the fixed version is
// in place before any field runs its own init() (those run in $(document).ready
// callbacks registered later, from the form body). See OpenCast #535.
(function ($) {
	var fixed_file_input = il.UI.Input.File;
	$(function () {
		il.UI.Input.File = fixed_file_input;
	});
})(jQuery);
