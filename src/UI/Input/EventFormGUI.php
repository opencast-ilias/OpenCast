...
class EventFormGUI extends ilPropertyFormGUI
{
    ...
    public function __construct(
        ...
        $this->cmd_url_upload_chunks = $cmd_url_upload_chunks ?? self::dic()->ctrl()->getLinkTarget($parent_gui,
                self::PARENT_CMD_UPLOAD_CHUNKS);
    ...
    protected function initForm()
    {
        ...
        $te->setRequired(xoctConf::getConfig(xoctConf::F_PRESENTER_MANDATORY) || $this->schedule);
        $this->addItem($te);
        ...
        if ($this->schedule) {
            ...
            $subinput = new ilInteractiveVideoTimePicker($this->txt(self::F_MULTIPLE_START_TIME),
                self::F_MULTIPLE_START_TIME);
            $subinput->setRequired(true);
            $opt->addSubItem($subinput);

            $subinput = new ilInteractiveVideoTimePicker($this->txt(self::F_MULTIPLE_END_TIME),
                self::F_MULTIPLE_END_TIME);
            $subinput->setRequired(true);
            $opt->addSubItem($subinput);
    ...
    public function fillObject() : bool
    {
        ...
        is_null($this->xoctOpenCast) ?
            ...
            $this->object->setWorkflowParametersForObjId((array) $this->getInput(self::F_WORKFLOW_PARAMETER),
                $this->xoctOpenCast->getObjId(), ilObjOpenCastAccess::hasPermission('edit_videos'));
        ...
        if ($this->getInput(self::F_MULTIPLE)) {
            ...
            if (!$date_and_location_disabled) {
                ...
                $default_datetime = $this->object->getDefaultDateTimeObject($start->get(IL_CAL_ISO_8601, '',
                    ilTimeZone::_getInstance()->getIdentifier()));
    ...
    protected function initButtons()
    {
        switch (true) {
            ...
            case $this->is_new and $this->schedule:
                ...
                $this->addCommandButton(self::PARENT_CMD_CREATE_SCHEDULED,
                    $this->txt(self::PARENT_CMD_CREATE_SCHEDULED));
                $this->addCommandButton(self::PARENT_CMD_CANCEL, $this->txt(self::PARENT_CMD_CANCEL));
                break;
            case  !$this->is_new:
    ...
    protected function checkAndShowConflictMessage(xoctException $e) : bool
    {
        if ($e->getCode() == xoctException::API_CALL_STATUS_409) {
            ...
            foreach ($conflicts as $conflict) {
                $message .= '<br>' . $conflict['title'] . '<br>' . date('Y.m.d H:i:s',
