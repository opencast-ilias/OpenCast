...
class PaellaConfigService
{
    ...
    public function getEffectivePaellaPlayerUrl(): array
    {
        ...
        $result = [
            'url' => $default_path,
            'info' => 'ILIAS-Paella: default ILIAS config used',
            'warn' => false
        ];
        if ($option === PluginConfig::PAELLA_OPTION_URL) {
            ...
            $result['url'] = $url;
            $result['info'] = 'ILIAS-Paella: config fetched from url';
    ...
    public function getPaellaPlayerThemeUrl(bool $live): array
    {
        ...
        $result = [
            ...
            'info' => 'ILIAS-Paella: default opencast theme used',
        ];
        if ($live) {
            if ($default_theme_live === PluginConfig::PAELLA_OPTION_URL) {
                ...
                $result['info'] = 'ILIAS-Paella: external live theme fetched from url';

                $reachable = $this->checkUrlReachable($url);
                if (!$reachable) {
                    xoctLog::getInstance()->writeWarning('url for paella live theme unreachable: ' . $url);
                    $result['theme_live_url'] = $default_theme_live_url;
                    $result['info'] = 'ILIAS-Paella: url for paella live theme unreachable, fallback to default live theme';
                }
            }
            ...
            $result['info'] = 'ILIAS-Paella: external theme fetched from url';
            $reachable = $this->checkUrlReachable($url);
            if (!$reachable) {
                xoctLog::getInstance()->writeWarning('url for paella theme unreachable: ' . $url);
                $result['theme_url'] = $default_theme_url;
                $result['info'] = 'ILIAS-Paella: url for paella theme unreachable, fallback to default theme';
            }
        }
        return $result;
    }
