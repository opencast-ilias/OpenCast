...
class ilOpenCastUpdateRBACPermsListObjective extends ilSetupObjective /* Setup\Objective */
{
    ...
    public function achieve(Environment $environment): Environment
    {
        ...
        while ($row = $db->fetchAssoc($set)) {
            ...
            if (method_exists($parent_obj, 'getDefaultAdminRole')) {
                ...
                } elseif ($no_download && in_array($download_op_id, $admin_ops_ids)) { // download NOT allowed, remove download op
            ...
            if (method_exists($parent_obj, 'getDefaultTutorRole')) {
                ...
                } elseif ($no_download && in_array($download_op_id, $tutor_ops_ids)) { // download NOT allowed, remove download op
            ...
            if (method_exists($parent_obj, 'getDefaultMemberRole')) {
                ...
                } elseif ($no_download && in_array($download_op_id, $member_ops_ids)) { // download NOT allowed, remove download op
    ...
    private function getActiveOperationsOfRolePureDB(int $ref_id, int $role_id): array
    {
        ...
        if ($row = $db->fetchAssoc($set)) {
            return unserialize($row['ops_id'], ['allowed_classes' => false]);
        }
        return [];
    }
