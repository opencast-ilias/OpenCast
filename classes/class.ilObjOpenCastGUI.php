...
class ilObjOpenCastGUI extends ilObjectPluginGUI
{
    ...
    protected function setTabs()
    {
        ...
        self::dic()->tabs()->addTab(self::TAB_EVENTS, self::plugin()->translate('tab_event_index'),
            self::dic()->ctrl()->getLinkTargetByClass(xoctEventGUI::class, xoctEventGUI::CMD_STANDARD));
        self::dic()->tabs()->addTab(self::TAB_INFO, self::plugin()->translate('tab_info'),
            self::dic()->ctrl()->getLinkTarget($this, 'infoScreen'));

        if (ilObjOpenCastAccess::checkAction(ilObjOpenCastAccess::ACTION_EDIT_SETTINGS)) {
            self::dic()->tabs()->addTab(self::TAB_SETTINGS, self::plugin()->translate('tab_series_settings'),
                self::dic()->ctrl()->getLinkTargetByClass(xoctSeriesGUI::class, xoctSeriesGUI::CMD_EDIT_GENERAL));
        }

        if ($xoctOpenCast->getPermissionPerClip() && ilObjOpenCastAccess::hasPermission('read')) {
            self::dic()->tabs()->addTab(self::TAB_GROUPS, self::plugin()->translate('tab_groups'),
                self::dic()->ctrl()->getLinkTarget(new xoctIVTGroupGUI()));
        }
        if (self::dic()->user()->getId() == 6 and ilObjOpenCast::DEV) {
            self::dic()->tabs()->addTab('migrate_event', self::plugin()->translate('tab_migrate_event'),
                self::dic()->ctrl()->getLinkTargetByClass(xoctEventGUI::class, 'search'));
            self::dic()->tabs()->addTab('list_all', self::plugin()->translate('tab_list_all'),
                self::dic()->ctrl()->getLinkTargetByClass(xoctEventGUI::class, 'listAll'));
        }

        if ($this->checkPermissionBool("edit_permission")) {
            self::dic()->tabs()->addTab("perm_settings", self::dic()->language()->txt("perm_settings"),
                self::dic()->ctrl()->getLinkTargetByClass(array(
                    get_class($this),
                    "ilpermissiongui",
                ), "perm"));
        }
    ...
    private function showEula()
    {
        ...
        self::dic()->mainTemplate()->setContent(xoctConf::getConfig(xoctConf::F_EULA));
    }
    ...
    public function afterSave(ilObject $newObj)
    {
        ...
        if ($channel_type == xoctSeriesFormGUI::EXISTING_NO) {
            ...
            $series->addContributor(self::dic()->user()->getFirstname() . ' ' . self::dic()->user()->getLastname(),
                true);
        }
        $series->update();
    ...
    protected function initHeader($render_locator = true)
    {
        ...
        if ($xoctOpenCast instanceof xoctOpenCast && $this->object) {
            ...
            if (self::dic()->access()->checkAccess('read', '', $_GET['ref_id'])) {
                self::dic()->history()->addItem($_GET['ref_id'],
                    self::dic()->ctrl()->getLinkTarget($this, $this->getStandardCmd()), $this->getType(),
    ...
    function infoScreen()
    {
        ...
        if ($refs = $xoctOpenCast->getDuplicatesOnSystem()) {
            ...
            foreach ($refs as $ref) {
                ...
                $info->addProperty(($i) . '. '
                    . self::plugin()->translate('info_linked_item'),
                    ilObject2::_lookupTitle(ilObject2::_lookupObjId($parent)), ilLink::_getStaticLink($parent));
                $i++;
            }
        }
        ...
        if ($xoctOpenCast->getVideoPortalLink() && $xoctOpenCast->getSeries()->isPublishedOnVideoPortal()) {
            ...
            $info->addProperty(self::plugin()->translate('series_video_portal_link', '',
                [xoctConf::getConfig(xoctConf::F_VIDEO_PORTAL_TITLE)]), $xoctOpenCast->getVideoPortalLink());
        }
    ...
    function handleMultiReferences($a_obj_id, $a_ref_id, $a_form_name)
    {
        ...
        if ($all_refs = $xoctOpenCast->getDuplicatesOnSystem()) {
            ...
            if ($may_delete_any) {
                $tpl->setVariable("TXT_INSTRUCTION",
                    self::dic()->language()->txt("rep_multiple_reference_deletion_instruction"));
            }
            ...
            if ($counter) {
                ...
                $tpl->setVariable("TXT_ADDITIONAL_INFO",
                    sprintf(self::dic()->language()->txt("rep_object_references_cannot_be_read"), $counter));
                $tpl->parseCurrentBlock();
            }
        ...
        foreach ($ref_ids as $ref_id) {
            ...
            foreach ($path_full as $idx => $data) {
                ...
                if ($ref_id != $data['ref_id']) {
                    ...
                    $path .= ('<a target="_top" href="' . ilLink::_getLink($data['ref_id'],
                            $data['type']) . '">' . $data['title'] . '</a>');
                }
            }
