...
class xoctEventGUI extends xoctGUI
{
    ...
    public function executeCommand()
    {
        ...
        switch ($nextClass) {
            case strtolower(xoctPlayerGUI::class):
                ...
                // check access
                if (!ilObjOpenCastAccess::hasReadAccessOnEvent($xoctEvent, xoctUser::getInstance(self::dic()->user()),
                    $this->xoctOpenCast)) {
        ...
        switch (xoctUserSettings::getViewTypeForUser(self::dic()->user()->getId(), filter_input(INPUT_GET, 'ref_id'))) {
            ...
            default:
                throw new xoctException(xoctException::INTERNAL_ERROR, 'Invalid view type ' .
                    xoctUserSettings::getViewTypeForUser(self::dic()->user()->getId(),
    ...
    protected function initViewSwitcherHTML($active)
    {
        if ($this->xoctOpenCast->isViewChangeable()) {
            ...
            $actions = [
                self::plugin()->translate('list') => self::dic()->ctrl()->getLinkTarget($this,
                    self::CMD_SWITCH_TO_LIST),
                self::plugin()->translate('tiles') => self::dic()->ctrl()->getLinkTarget($this,
                    self::CMD_SWITCH_TO_TILES),
            ];

            $aria_label = self::plugin()->translate('info_view_switcher');
            $view_control = $f->viewControl()->mode($actions,
                $aria_label)->withActive(self::plugin()->translate($active));
    ...
    {
        xoctUserSettings::changeViewType(self::dic()->user()->getId(), filter_input(INPUT_GET, 'ref_id'),
            xoctUserSettings::VIEW_TYPE_TILES);
    ...
    {
        xoctUserSettings::changeViewType(self::dic()->user()->getId(), filter_input(INPUT_GET, 'ref_id'),
            xoctUserSettings::VIEW_TYPE_LIST);
        self::dic()->ctrl()->redirect($this, self::CMD_STANDARD);
    }
    ...
    protected function changeTileLimit()
    {
        ...
        if (in_array($tile_limit, [4, 8, 12, 16])) {
            xoctUserSettings::changeTileLimit(self::dic()->user()->getId(), filter_input(INPUT_GET, 'ref_id'),
                $tile_limit);
        }
        self::dic()->ctrl()->redirect($this, self::CMD_STANDARD);
    }
    ...
    protected function create()
    {
        ...
        $xoctAclStandardSets = new xoctAclStandardSets($xoctUser->getOwnerRoleName() ? array($xoctUser->getOwnerRoleName(),
                                                                                             $xoctUser->getUserRoleName()
        ) : array());
    ...
    public function getModals() : EventModals
    {
        if (is_null($this->modals)) {
            $modals = new EventModals($this, self::plugin()->getPluginObject(), self::dic()->dic(),
    ...
    protected function getIntroTextHTML()
    {
        ...
        if ($this->xoctOpenCast->getIntroductionText()) {
            $intro = new ilTemplate('./Customizing/global/plugins/Services/Repository/RepositoryObject/OpenCast/templates/default/tpl.intro.html',
                '', true, true);
