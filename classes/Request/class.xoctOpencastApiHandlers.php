...
class xoctOpencastApiHandlers
{
    ...
    {
        $classmethods = get_class_methods('xoctOpencastApiHandlers');
        // Request middlewares.
        ...
        return function (callable $handler) {
            return function (RequestInterface $request, array $options) use ($handler) {
                ...
                $isExcluded = array_filter(self::excludedRequests(), function ($excReq) use ($path, $query, $method) {
                    return ltrim($excReq['path'], '/') === ltrim($path, '/') && $excReq['query'] === $query && $excReq['method'] === $method;
                });
                if (!empty($isExcluded)) {
    ...
    private static function statsCallback()
    {
        return function (TransferStats $stats) {
            ...
            xoctLog::getInstance()->write('CONNECT_TIME: ' . round($handlerStats["connect_time"] * $i, 2) . ' ms', xoctLog::DEBUG_LEVEL_1);
            xoctLog::getInstance()->write('NAMELOOKUP_TIME: ' . round($handlerStats["namelookup_time"] * $i, 2) . ' ms', xoctLog::DEBUG_LEVEL_1);
            xoctLog::getInstance()->write('REDIRECT_TIME: ' . round($handlerStats["redirect_time"] * $i, 2) . ' ms', xoctLog::DEBUG_LEVEL_1);
            xoctLog::getInstance()->write('STARTTRANSFER_TIME: ' . round($handlerStats["starttransfer_time"] * $i, 2) . ' ms', xoctLog::DEBUG_LEVEL_1);
            xoctLog::getInstance()->write('PRETRANSFER_TIME: ' . round($handlerStats["pretransfer_time"] * $i, 2) . ' ms', xoctLog::DEBUG_LEVEL_1);
            xoctLog::getInstance()->write('TOTAL_TIME: ' . round($handlerStats["total_time"] * $i, 2) . ' ms', xoctLog::DEBUG_LEVEL_1);
        };
    }
